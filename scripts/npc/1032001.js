...
function action(mode, type, selection) {
    ...
    if (actionx["1stJob"]) {
        if (status == 0) {
            ...
            cm.sendNextPrev("你现在变得更强大了,我给了你一点点#b能力值#k，并增加了你的一些背包空间. 当你打开#b技能#k 菜单在屏幕的左下角,您可以使用SP'.不过,有一个警告:你不可能一下子把它都提起来.也有一些技能只有在你先学会了一些技能之后才能获得.");
        } else if (status == 3) {
            cm.sendNextPrev("记住，技能非常重要。你的属性值也可以提升你的魔法威力，魔法师的主属性是智力，副属性是运气，如果不知道该怎么点，可以点#b自动分配#k");
        } else if (status == 4) {
            cm.sendNextPrev("现在提醒你。从现在开始你每次死亡都会失去一些经验值。你要额外注意这点，魔法师的生命值是最少的");
        } else if (status == 5) {
            cm.sendNextPrev("我能教你的就这么多了，希望你一路好运，年轻的魔法师");
        } else {
            cm.dispose();
        }
        ...
        if (status == 0) {
            ...
            if (!cm.haveItem(4031012)) {
                ...
                if (selection < 3) {
                    if (selection == 0) {
                        ...
                        cm.sendNext("牧师擅长使用神圣魔法,可以为队友治疗,可以给队友提供很强的增益法术.");    //cleric
                    }
            ...
            job += selection * 10;
            cm.sendYesNo("你想转职成为" + (job == 210 ? "#b巫师(火/毒)#k" : job == 220 ? "#b巫师(冰/雷)#k" : "#b牧师#k") + "? 二转职业一旦选择就不能反悔了,确定吗?");
        } else if (status == 3) {
            if (cm.haveItem(4031012)) {
                cm.gainItem(4031012, -1);
            }
            cm.completeQuest(100008);
            cm.sendNext("好的,你现在是一个" + (job == 210 ? "#b巫师(火/毒)#k" : job == 220 ? "#b巫师(冰/雷)#k" : "#b牧师#k") + "了.。法师和巫师是一群聪明的人，拥有不可思议的魔法能力，能够轻松刺穿怪物的肉体和心灵......请每天训练自己。当你需要变得更强的时候，我会帮助你。");
            if (cm.getJobId() != job) {
                cm.changeJobById(job);
            }
        } else if (status == 4) {
            cm.sendNextPrev("我给你了一本技能书，里面有一些你作为." + (job == 210 ? "#b巫师(火/毒)#k" : job == 220 ? "#b巫师(冰/雷)#k" : "#b牧师#k") + "的技能。我还增加了你的其它栏的一行空间，你的HP和MP都提升了");
        } else if (status == 5) {
            cm.sendNextPrev("我还给了你一点技能点，你现在可以使用二转技能了，打开技能菜单查看吧。但是你无法一次把它们都升级，有的技能需要先学习基础技能");
        } else if (status == 6) {
            cm.sendNextPrev((job == 210 ? "#b巫师(火/毒)#k" : job == 220 ? "#b巫师(冰/雷)#k" : "#b牧师#k") + "需要坚强，但是记住你不能使用你的力量去欺凌弱小，如果你的力量没有用在正道上，你将难以变得更强。当你想要更近一步的时候，可以来找我，我会在这等你");
        }
