        ...
        if (status == 1) {
            ...
            if (selection == 0) {
                ...
                if (cm.getPlayer().getGuildId() < 1 || cm.getPlayer().getGuildRank() != 1) {
                    ...
                    var Guild = Java.type("net.server.guild.MapleGuild");  // thanks Conrad for noticing an issue due to call on a static method here
                    cm.sendYesNo("增加#b5#k家族容量将花费#b " + Guild.getIncreaseGuildCost(cm.getPlayer().getGuild().getCapacity()) + " 金币#k,继续吗?");
                }
            }
        } else if (status == 2) {
