    ...
    if (actionx["Mental"]) {
        if (status == 0) {
            ...
            if (Math.floor(cm.getJobId() / 10) == 41) {
                cm.sendNext("恭喜你，你现在是#b无影人#k了. 这本技能书为隐士介绍了一系列新的攻击技能，使用阴影作为复制和替换的方式，包括以下技能#b金钱投掷#k(用金币替换魔法值，用基于金币投掷量的伤害攻击怪物)和#b影分身#k(创造一个模仿每个动作的阴影，使一个无影人攻击一个怪物，就像两个无影人在那里一样).使用这些技能来对付以前很难征服的怪物。");
            } else {
                cm.sendNext("恭喜你，你现在是#b独行客#k了. 技能书中新增的一项技能叫做#b分身术#k,其中你可以召唤同伴一次攻击多个怪物。独行客也可以通过多种方式利用金币，攻击怪物(#b金钱炸弹#k,爆炸地面上的金币),防御(#b金钱护盾#k,减少怪物伤害).");
            }
