        ...
        if (!eim.isEventCleared()) {
            if (status == 0) {
                if (eim.getIntProperty("npcShocked") == 0 && cm.haveItem(4001131, 1)) {
                    ...
                    if (cm.isEventLeader()) {
                        ...
                        cm.sendNext("太好了!蒙特鸠和蒙特鸠的实验资料都被你弄到手了.我们可以继续了.");
            ...
            if (status == 0) {
                if (eim.getIntProperty("escortFail") == 0) {
                    cm.sendNext("最后，朱丽叶安全了！多亏你的努力，我们才能把她从犹泰的手中解救出来，犹泰现在将因反叛玛加提亚而受到审判。从现在起，他将开始康复，我们将继续关注他的努力，确保他不会再给未来带来麻烦.");
                } else {
                    cm.sendNext("朱丽叶现在安全了，虽然战斗给她带来了损失。。。多亏你的努力，我们才能把她从犹泰的手中解救出来，犹泰现在将因反叛玛加提亚而受到审判。谢谢你.");
                    status = 2;
                }
            } else if (status == 1) {
