...
function action(mode, type, selection) {
    ...
    if (actionx["1stJob"]) {
        ...
        if (status == 0) {
            ...
            job += selection * 10;
            cm.sendYesNo("那么你是想成为" + (job == 310 ? "#b猎人(弓)#k" : "#b弩弓手(弩)#k") + "?你要知道二转职业一旦决定了就无法反悔了,确定吗?");
        } else if (status == 3) {
            if (cm.haveItem(4031012)) {
                cm.gainItem(4031012, -1);
            }

            cm.sendNext("好的,你已经是一个 " + (job == 310 ? "#b猎人(弓)#k" : "#b弩弓手(弩)#k") + "了." + (job == 310 ? "#b猎人#k" : "#b弩弓手#k") + "有着不可思议的视力和敏捷的身手，能够轻易地把箭刺穿怪物的心脏。。。 请每天训练自己。我会帮助你变得比现在更强大。");
            if (cm.getJobId() != job) {
                cm.changeJobById(job);
            }
        } else if (status == 4) {
            cm.sendNextPrev("我刚刚给了你一本书，上面列出了你作为" + (job == 310 ? "猎人" : "弩弓手") + "的技能书.还增加了一些你的背包栏位，你的HP和MP也提升了");
        } else if (status == 5) {
            cm.sendNextPrev("我还给了你一些技能点.打开技能面板你就可以学习二转技能了.不过，还是要提醒一下,这些技能并不是一开始就全部就能使用.有一些技能是依赖其他技能的.");
        } else if (status == 6) {
            cm.sendNextPrev((job == 310 ? "猎人" : "弩弓手") + "需要坚强，但是记住你不能使用你的力量去欺凌弱小，如果你的力量没有用在正道上，你将难以变得更强。当你想要更近一步的时候，可以来找我，我会在这等你");
        }
