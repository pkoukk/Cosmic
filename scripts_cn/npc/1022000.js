...
function start() {
    ...
    if (parseInt(cm.getJobId() / 100) == jobType && cm.canSpawnPlayerNpc(GameConstants.getHallOfFameMapid(cm.getJob()))) {
        ...
        if (cm.getJobId() == 0) {
            actionx["1stJob"] = true;
            cm.sendNext("你想成为#r战士#k?这是需要一些条件的#b你至少得10级,并且" + cm.getFirstJobStatRequirement(jobType) + "#k.让我看看..");   // thanks Vcoc for noticing a need to state and check requirements on first job adv starting message
    ...
    if (actionx["1stJob"]) {
        if (status == 0) {
            ...
            if (cm.canHold(1302077)) {
                ...
                cm.sendNext("从现在开始,你就是一个合格的战士了.这个职业并不轻松,但是如果你努力练习加强你的力量和技能,你可以克服碰到的任何困难.加油吧!");
            } else {
                cm.sendNext("给背包腾出一些空间然后再来和我对话.");
                cm.dispose();
            }
        } else if (status == 2) {
        ...
        if (status == 0) {
            ...
            job += selection * 10;
            cm.sendYesNo("确定要转职为" + (job == 110 ? "#b剑客#k" : job == 120 ? "#b准骑士#k" : "#b枪战士#k") + "?你要知道二转职业一旦决定了就无法反悔了,确定吗?");
        } else if (status == 3) {
            if (cm.haveItem(4031012)) {
            ...
            cm.sendNextPrev("我刚刚给你了一本作为" + (job == 110 ? "剑客" : job == 120 ? "准骑士" : "枪战士") + "的技能书.还增加了一些你的背包栏位，你的HP和MP也提升了");
        } else if (status == 5) {
            cm.sendNextPrev("我还给你增加了一些技能点.打开#b技能面板#k.你可以学习新的技能了，有些技能无法立刻学习，你得想学习别的基础技能");
        } else if (status == 6) {
            cm.sendNextPrev((job == 110 ? "剑客" : job == 120 ? "准骑士" : "枪战士") + "需要坚强，但是记住你不能使用你的力量去欺凌弱小，如果你的力量没有用在正道上，你将难以变得更强。当你想要更近一步的时候，可以来找我，我会在这等你");
        }
        ...
        if (status == 0) {
            ...
            cm.sendNextPrev("他是我的分身,你将会陷入苦战.他会使用你可能都没有见过的技能，你必须一对一战胜他.秘密通道有时间限制,你必须在时间结束前打败他.祝你好运,希望你能安全带回#b#t4031059##k.");
        }
