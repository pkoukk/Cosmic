    ...
    if (actionx["Mental"]) {
        if (status == 0) {
            ...
            if (Math.floor(cm.getJobId() / 10) == 31) {
                cm.sendNext("恭喜你，你现在是#b射手#k了. 你真正需要掌握的技能之一是#b贯穿箭#k它允许射手近距离射箭。#b烈火箭#k允许游侠暂时对怪物进行基于火力的攻击，而技能就像#b替身术#k(召唤稻草人吸引怪物的注意力)和#b银鹰召唤#k(召唤一只攻击怪物的银鹰)增加弓箭手的远程攻击等级。");
            } else {
                cm.sendNext("恭喜你，你现在是#b游侠#k了.你真正需要掌握的技能之一是#b贯穿箭#k它允许游侠近距离射箭。#b寒冰箭#k允许游侠暂时对怪物进行冰属性攻击，而技能如#b替身术t#k(召唤稻草人吸引怪物的注意力)和#b金鹰召唤#k(召唤一只攻击怪物的金鹰)增加弓箭手的远程攻击等级。");
            }
        } else if (status == 3) {
