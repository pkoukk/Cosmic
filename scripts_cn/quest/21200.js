...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendOk("你的#b#p1201001###b#m140000000##k突然变得奇怪起来。据记载，战斧在召唤主人的时候会这样做。#b它在召唤你#k。请回到岛上，检查一下。");
    } else if (status == 2) {
        qm.startQuest();
        qm.dispose();
    }
...
function end(mode, type, selection) {
    ...
    if (status == 0) {
        qm.sendNext("嗡 嗡 嗡 嗡 嗡....");
    }//Giant Polearm
    else if (status == 1) {
        qm.sendNextPrev("#b(#p1201001#产生了一个起伏的回声。但是那边的那个男孩是谁?)", 2);
    } else if (status == 2) {
        ...
        qm.sendNextPrev("战神,你现在听到我说话了吗？是我！你不认识我吗？我是你的武器，#b#p1201002#战斧#k!");
    } else if (status == 8) {
        ...
        qm.sendYesNo("对不起，说什么都没有用！你知道几百年来我是多么孤独和无聊吗？我不在乎发生了什么！记起我！现在快点记起我!");
    } else if (status == 12) {
        qm.completeQuest();
        qm.sendNext("#b(自称是#p1201002#这个#p1201001#在沮丧中大叫, 你不认为这次谈话会有任何进展。你最好去和#p1201000#先谈谈.)", 2);
