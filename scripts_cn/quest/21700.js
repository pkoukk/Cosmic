...
function start(mode, type, selection) {
    ...
    if (mode == 1) {
        ...
        if (status == 4) {
            qm.sendNext("不你是说你可以自己训练吗？我只是想让你知道，如果你和教练一起训练，你会得到更好的结果。你不能独自生活在这个世界上。你必须学会与他人相处。");
            qm.dispose();
            return;
        }
        status--;
    ...
    if (status == 0) {
        qm.sendNext("你似乎在回想什么。这个战斧果然认出了你。那么你肯定就是#b使用战斧的英雄，战神#k了。你想起什么其他的了吗？有关战斧的技能之类......");
    } else if (status == 1) {
        ...
        qm.sendNext("武器要是能使得更熟练就好了。送你一支#b战斧#k，希望你在修炼的时候能够进步得更快。带着这支战斧......");
